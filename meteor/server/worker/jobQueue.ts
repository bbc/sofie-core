...
export class WorkerJobQueueManager {
	...
	async waitForNextJob(queueName: string): Promise<void> {
		...
		if (queue.jobsHighPriority.length > 0 || queue.jobsLowPriority.length > 0) {
			return
		}
	...
	#queueJobInner(queueName: string, jobToQueue: JobEntry, options?: QueueJobOptions): void {
		...
		const queue = this.#getOrCreateQueue(queueName)
		const priorityQueue = options?.lowPriority ? queue.jobsLowPriority : queue.jobsHighPriority

		// Debounce: skip if an identical job is already queued
		// nocommit: respect the debounce time
		if (options?.debounce) {
			const alreadyQueued = priorityQueue.some(
