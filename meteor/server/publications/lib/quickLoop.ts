...
export function modifyPartForQuickLoop(
	...
	if (isLoopingOverriden && (part.durations.expectedDuration ?? 0) < fallbackPartDuration) {
	...
	part.autoNext = part.autoNext || (isLoopingOverriden && (part.durations.expectedDuration ?? 0) > 0)
}
...
export function modifyPartInstanceForQuickLoop(
	...
	const canAutoNext = () => {
		...
		if (start !== undefined && partInstance.part.durations.expectedDuration) {
			...
			if (partInstance.part.durations.expectedDuration - playbackDuration < 0) {
				return false
			}
		}
