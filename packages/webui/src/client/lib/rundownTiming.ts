	...
	updateDurations(
				...
				if (lastStartedPlayback && !partInstance.timings?.duration) {
					...
					const duration =
						...
						(partInstance.timings?.plannedStoppedPlayback
							...
							: undefined)
					partDuration =
						Math.max(duration || partDurations.expectedDurationWithTransition, now - lastStartedPlayback) -
						playOffset
		...
		if (currentAIndex >= 0) {
			...
			if (
				...
				(currentLivePart.durations.expectedDuration === undefined ||
					currentLivePart.durations.expectedDuration === 0)
			) {
				onAirPartDuration =
